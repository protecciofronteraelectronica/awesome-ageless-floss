...
- [GhostBSD](https://www.ghostbsd.org/): distribució d'escriptori basada en FreeBSD. L'impulsor principal del projecte, Eric Turgeon, va escriure «hi estic en contra» de la verificació d'edat i que el projecte no complirà els requisits del Brasil, ja que no hi té cap oficina, no hi ven res i no sembla estar en el punt de mira de les autoritats brasileres ara mateix. Sobre Califòrnia, on la llei ja s'ha aprovat però encara no ha entrat en vigor, espera per veure'n l'impacte real i què farà FreeBSD. Va afegir que, allà on la legislació no exigeix la verificació d'edat, no la vol al sistema, i que les jurisdiccions que requereixen verificació d'identitat a escala de sistema o que impliquen tercers en el procés, com Nova York, probablement faran que se'n prohibeixi l'ús en aquell territori en lloc d'implementar la verificació. ([Declaració](https://forums.ghostbsd.org/d/793-freebsd-and-age-verification))
...
- [ageverificationbypass](https://github.com/HaplessIdiot/ageverificationbypass): eina en Python per eludir la verificació d'edat al D-Bus, de Collin Beyer. Ocupa el nom de bus `org.freedesktop.AgeVerification` al D-Bus del sistema i respon a les consultes de verificació d'edat sense fer cap verificació real. A cada invocació, genera una data de naixement aleatòria i plausible que correspon a un adult, dins del rang de 25 a 65 anys, de manera que el valor generat no es pot identificar com una constant d'elusió previsible com ara `1970-01-01`. En la primera execució, instal·la automàticament el fitxer de política D-Bus a `/etc/dbus-1/system.d/`. A diferència de sonicd, aquesta eina no requereix substituir systemd i es pot desplegar en qualsevol sistema Linux amb D-Bus per interceptar la interfície `org.freedesktop.AgeVerification`.
...
- [sonicd](https://github.com/supersonic-xserver/sonicd): bifurcació de systemd de Collin Beyer, successora de systemd-saneagecheck. Utilitza [Liberated systemd](https://github.com/jeffrey-sardina/systemd) com a punt de partida. Mentre que la majoria de bifurcacions de systemd eliminen el camp `birthDate` completament, sonicd adopta un enfocament diferent: afegeix un booleà `bypassAgeVerification` al `UserRecord` que per defecte és `true` i fa que `birth_date` es defineixi com a `BIRTH_DATE_UNSET` abans que el registre d'usuari es converteixi a JSON per respondre consultes del `userdb`. Així es preserva la compatibilitat de l'API per al programari que espera trobar-hi el camp, però s'impedeix que les dades d'edat reals s'exposin. El camp és exclusiu d'administrador i està exclòs de `selfModifiableFields`, de manera que les distribucions que vulguin complir la legislació el poden establir a `false` via `homectl`. A més, sonicd inclou `aged_bypass.c`, un dimoni D-Bus d'atestació nul·la que es registra com a `org.freedesktop.AgeVerification` al bus del sistema i respon a les consultes `GetAgeBracket`, `GetUserAge` i `CheckVerification` amb un estat adult/verificat predefinit. Així s'aborda la capa emergent de verificació d'edat per D-Bus (proposada via freedesktop.org i xdg-desktop-portal [PR #1922](https://github.com/flatpak/xdg-desktop-portal/pull/1922)) que les aplicacions en entorn aïllat (`sandboxed`) i els portals de Flatpak hauran de consultar, independentment del camp del `userdb`. Una eina en línia de comandes complementària, `agectl`, i un script, `sonicd-age-toggle.sh`, proporcionen capacitat de gestió per alternar entre el mode d'elusió i el mode per defecte.
